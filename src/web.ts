  ...
  implements SecureStoragePlugin
{
  ...
  async setStorageAdapter(options: {
    web: { adapter: WebStorageAdapter };
  ...
  async getStorageAdapter(): Promise<{ adapter: WebStorageAdapter }> {
    const adapter = (localStorage.getItem(this.STORAGE_ADAPTER_KEY) ||
      WebStorageAdapter.LocalStorage) as WebStorageAdapter;
  ...
  getAccessibility(/*_options: { key: string }*/): Promise<{
